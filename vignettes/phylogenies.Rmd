...
Very cool! Note that `guide_geo()` can actually be used for any plot, but it basically does the same thing as `coord_geo()` for non-radial plots and generally requires more lines of code. Also, as with `coord_geo_radial()`, `guide_geo()` only works with ggplot2 version 3.5.0 and higher.
