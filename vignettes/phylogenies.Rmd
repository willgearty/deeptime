...
__deeptime__ also provides a specialized geom, `geom_text_clade()`, for adding clade labels to phylogenies plotted with `ggtree()`. It works similarly to `ggtree::geom_cladelab()`. However, unlike that geom, this geom is intended to work with all coordinate systems, including `coord_geo()`, `ggplot2::coord_radial()`, and `coord_geo_radial()`. Here's a brief example using the same mammal phylogeny from above. Notice that we use `position = position_nudge()` to move the labels away from the tips a bit and `hjust` to move the label text away from the label bar. We also use `plot.margin` to add some space around the plot so the labels aren't cut off.
...
<div class="figure">
<img src="phylogenies-7.png" alt="plot of mammal phylogeny with stacked guides, coord_geo_radial, and clade labels generated with geom_text_clade" width="100%" />
</div>

You can also use a data frame and the `ggfun::%<+%()` operator to merge the data with the tree data. This allows you to map aesthetics to columns in the data frame.
...
<div class="figure">
<img src="phylogenies-8.png" alt="plot of primate phylogeny with stacked guides, coord_geo_radial, and clade labels generated with geom_text_clade" width="100%" />
</div>

`geom_text_clade()` inherits all of the arguments from `ggplot2::geom_text()` and `ggplot2::geom_linerange()`, so you can modify the text size, color, font, linewidth, etc. You can also use the `angle`, `hjust`, and `vjust` aesthetics to further customize the text orientation and justification.

Finally, while `geom_text_phylo()` and `geom_text_clade()` were designed to work with radial coordinate systems, they work just as well with Cartesian coordinate systems!
