...
### Disclaimer
Note that `coord_geo_polar()` has been tested with the plotting of ultrametric and non-ultrametric phylogenies with `ggtree`, but it may have unexpected behavior when combined with other functions from `ggtree`. Furthermore, `coord_geo_polar()` may be useful (albeit perhaps abstract) for plotting other types of data, but this has not been investigated.
</div>
