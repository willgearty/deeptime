...
data(mammal.tree)
data(mammal.data)
# Load ggrepel for adding labels
library(ggrepel)
```
...
## Phylomorphospaces
Often, trait data will be accompanied with a phylogeny. You may want to visualize both your phylogeny, the traits of your species, and the evolution of the trait along your phylogeny. To accomplish this, you can create a two-dimensional phylomorphospace. `phytools` has a function for accomplishing this in base R. The `geom_phylomorpho()` function in `deeptime` will help you accomplish this with `ggplot`. Note that the labels are used here for demonstration purposes and may not be desirable for finished products because they can obscure the phylogenetic relationships.
